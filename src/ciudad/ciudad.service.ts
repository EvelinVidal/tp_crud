...
export class CiudadService {
  ...
  async create(ciudadDTO: CiudadDto): Promise<boolean> {
    // falta verificar si la ciudad está o no en la bd
    try {
  ...
  async findById(id: number): Promise<Ciudad> {
    try {
      ...
      throw new HttpException(
        ...
        HttpStatus.NOT_FOUND,
      ); // http exception que permite controlar mejor los errores. Es un json  que encuentre el error y el status.
    }
  }
